...
void test_sort_matrix() {
    ...
    assert(sorted != NULL);
    assert(sorted->rows_indices[0] == 0 && sorted->coll_indices[0] == 0 && fabsf(sorted->values[0] - 2.0f) < 1e-5f);
    assert(sorted->rows_indices[1] == 0 && sorted->coll_indices[1] == 2 && fabsf(sorted->values[1] - 1.0f) < 1e-5f);
    assert(sorted->rows_indices[2] == 1 && sorted->coll_indices[2] == 0 && fabsf(sorted->values[2] - 3.0f) < 1e-5f);
    assert(sorted->rows_indices[3] == 2 && sorted->coll_indices[3] == 1 && fabsf(sorted->values[3] - 5.0f) < 1e-5f);
...
void test_multiplication_two_matrix() {
    ...
    int row_b[] = { 0, 1, 2, 2 };
    int col_b[] = { 0, 1, 0, 1 };
    float val_b[] = { 5.0f, 6.0f, 7.0f, 8.0f };
...
void test_coo_map() {
    ...
    assert(fabsf(matrix->values[0] - 4.0f) < 1e-5f);
    assert(fabsf(matrix->values[1] - 9.0f) < 1e-5f);
    assert(fabsf(matrix->values[2] - 16.0f) < 1e-5f);
    free_matrix(matrix);
}
...
void test_coo_map2() {
    ...
    int row2[] = { 0, 1, 1, 2 };
    int col2[] = { 0, 1, 2, 2 };
    float val2[] = { 5.0f, 6.0f, 7.0f, 8.0f };
    ...
    sort_matrix(result);
    assert(result->nnz == 5);
    assert(result->rows_indices[0] == 0 && result->coll_indices[0] == 0 && fabsf(result->values[0] - 6.0f) < 1e-5f);
    assert(result->rows_indices[1] == 0 && result->coll_indices[1] == 1 && fabsf(result->values[1] - 2.0f) < 1e-5f);
