...
let generateRandomSparseMatrix (rows: uint64) (cols: uint64) (density: float) (seed: int) =
    let rng = Random(seed)
...
let benchmarkMultiply rows cols density iterations =
    let m1 = generateRandomSparseMatrix rows cols density 42
    let m2 = generateRandomSparseMatrix cols rows density 43
...
let benchmarkVectorMatrix rows cols density iterations =
    let v = generateRandomVector rows 42
    let m = generateRandomSparseMatrix rows cols density 43
...
let main argv =
    if argv.Length <> 3 then
        ...
        let result =
            ...
            | "mv" -> benchmarkVectorMatrix size size density 5
            | _ -> 0.0
        printfn "%.6f" result
        0